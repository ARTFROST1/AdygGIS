...
**Дата:** 5 января 2026  
**Версия:** 1.0  
**Статус:** ✅ Реализовано в коде (документ частично исторический)

> Важно: этот документ начинался как «план рефакторинга». На текущий момент ключевые файлы (Retrofit API, remote data source, конфиг, DI) уже присутствуют в проекте.
> Некоторые фрагменты кода ниже могут отличаться от актуальной реализации — ориентируйтесь на реальные файлы в `app/src/main/java/...`.
...
|------|--------|----------|
| `app/src/main/java/com/adygyes/app/data/remote/config/SupabaseConfig.kt` | ✅ | Конфигурация Supabase |
| `app/src/main/java/com/adygyes/app/data/remote/api/SupabaseApiService.kt` | ✅ | Retrofit API interface |
| `app/src/main/java/com/adygyes/app/data/remote/dto/SyncMetadataDto.kt` | ✅ | DTO для tombstones |
| `app/src/main/java/com/adygyes/app/data/remote/SupabaseRemoteDataSource.kt` | ✅ | Remote data source |
| `app/src/main/java/com/adygyes/app/di/module/NetworkModule.kt` | ✅ | Hilt DI module |
...
Сервис синхронизации описан в [04_SYNC_SERVICE.md](04_SYNC_SERVICE.md) (также ориентироваться на фактические классы `data/sync/*` в коде).
