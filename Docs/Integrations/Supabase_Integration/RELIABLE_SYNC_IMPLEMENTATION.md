...
### 1. **Неправильный бюджет таймаутов**
- До: `readTimeout: 10s` ❌ (часто не хватало на сотовых)
- Сейчас (актуально по коду): `connectTimeout: 15s`, `readTimeout: 25s`, `writeTimeout: 15s`, `callTimeout: 120s`

**Проблема (как проявлялась):** на сотовых первый запрос мог либо оборваться слишком рано, либо «залипнуть» и блокировать цепочку вызовов.
...
- Нужен строгий HTTPS (через `network_security_config.xml`)
...
### 3. ⏱️ Оптимизированные таймауты для мобильных сетей
...
OkHttpClient.Builder()
    ...
    // Read timeout intentionally not huge to avoid "wait ~60s then retry" stalls.
    .readTimeout(25, TimeUnit.SECONDS)
    .writeTimeout(15, TimeUnit.SECONDS)
    // Total budget for request + retry.
    .callTimeout(120, TimeUnit.SECONDS)
```

**Обоснование:**
- `readTimeout` держим умеренным, чтобы не получать долгие «зависания» на первой попытке.
- `callTimeout` даёт общий бюджет, чтобы одна retry-попытка успевала завершиться.

---

### 4. 🌐 DNS: DoH (Cloudflare) + fallback

**Файл:** `NetworkModule.kt`

```kotlin
fun provideDns(): Dns {
    // Реализация в коде использует DoH (Cloudflare) для Supabase-хостов
    // и fallback на системный DNS, чтобы уменьшить проблемы с carrier DNS.
    // См. актуальную реализацию в NetworkModule.kt.
    return dns
}
```

**Решает:** Проблемы с медленным/нестабильным carrier DNS на сотовых операторах.
...
|----------|------|--------|-----------|
| **Connect timeout** | 15s | 15s | = |
| **Read timeout** | 10s | 25s | ✅ стабильнее |
| **Total timeout** | нет | 120s | ✅ Новое |
| **Retry logic** | ❌ Нет | ✅ 3 попытки | 🎯 |
| **Exponential backoff** | ❌ Нет | ✅ 1s→2s→4s | 🎯 |
| **Network check** | ❌ Нет | ✅ Да | 🎯 |
| **DNS (DoH + fallback)** | ❌ Нет | ✅ Да | 🎯 |
...
| **HTTPS security** | ⚠️ Не гарантировано | ✅ Строгий HTTPS | 🔒 |
