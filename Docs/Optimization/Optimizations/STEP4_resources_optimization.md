...
**Статус**: ⚠️ Можно минифицировать (опционально, legacy fallback/seed)
...
3. Если OK и вы оставляете JSON fallback - заменить оригинал
...
### 1️⃣ Минификация JSON (опционально, legacy fallback)
```bash
# Запустить скрипт:
minify_json.bat

# Если результат OK и вы хотите оптимизировать legacy fallback файл:
